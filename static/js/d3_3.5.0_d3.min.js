! function() {
	...
	function wt(n, t, e) {
		...
		var r = vt((.4124564 * n + .3575761 * t + .1804375 * e) / Ja),
			u = vt((.2126729 * n + .7151522 * t + .072175 * e) / Ga),
			i = vt((.0193339 * n + .119192 * t + .9503041 * e) / Ka);
		...
		function e(n) {
			...
			return {
				...
				point: function(f, h) {
					...
					if (!e && (l = c = v) && n.lineStart(), v !== c && (g = r(e, p), (_e(e, g) || _e(p, g)) && (p[0] += Ta, p[1] += Ta, v = t(p[0], p[1]))), v !== c) s = 0, v ? (n.lineStart(), g = r(p, e), n.point(g[0], g[1])) : (g = r(e, p), n.point(g[0], g[1]), n.lineEnd()), e = g;
					else if (a && e && o ^ v) {
						var m;
	...
	function Ir(n) {
		for (var t, e, r, u, i, o, a, c, l, s, f = n[0][0], h = n[1][0], g = n[0][1], p = n[1][1], v = el, d = v.length; d--;)
			if (i = v[d], i && i.prepare())
				for (a = i.edges, c = a.length, o = 0; c > o;) s = a[o].end(), r = s.x, u = s.y, l = a[++o % c].start(), t = l.x, e = l.y, (va(r - t) > Ta || va(u - e) > Ta) && (a.splice(o, 0, new nu(Kr(i.site, s, va(r - f) < Ta && p - u > Ta ? {
					x: f,
					y: va(t - f) < Ta ? e : p
				} : va(u - p) < Ta && h - r > Ta ? {
					x: va(e - p) < Ta ? t : h,
	...
	function ou(n, t) {
		...
		for (tl = [], el = new Array(n.length), rl = new tu, il = new tu;;)
			if (u = ul, i && (!u || i.y < u.y || i.y === u.y && i.x < u.x))(i.x !== e || i.y !== r) && (el[i.i] = new Yr(i), Fr(i), e = i.x, r = i.y), i = n.pop();
			else {
				if (!u) break;
				jr(u.arc)
			}
	...
	function Gu(n, t, e) {
		...
		if (n.charge = 0, !n.leaf)
			for (var i, o = n.nodes, a = o.length, c = -1; ++c < a;) i = o[c], null != i && (Gu(i, t, e), n.charge += i.charge, r += i.charge * i.cx, u += i.charge * i.cy);
		if (n.point) {
	...
	function xi(n) {
		...
		if ((e = n.children) && (l = e.length)) {
			var e, r, u, i, o, a, c, l, s = 1 / 0,
				...
				h = 1 / 0,
				g = -1 / 0;
			if (e.forEach(bi), r = e[0], r.x = -r.r, r.y = 0, t(r), l > 1 && (u = e[1], u.x = u.r, u.y = 0, t(u), l > 2))
				for (i = e[2], Si(r, u, i), t(i), mi(r, i), r._pack_prev = i, mi(i, u), u = r._pack_next, o = 3; l > o; o++) {
					Si(r, u, i = e[o]);
					var p = 0,
						v = 1,
						d = 1;
	...
	function Bi(n, t, e) {
		...
		if (e) {
			...
			if (u.shift(), "s" === u[8]) {
				...
				return u[7] || (u[7] = "." + Wi(i.scale(r[2]))), u[8] = "f", e = ta.format(u.join("")),
					function(n) {
	...
	ta.interpolateTransform = Ou, ta.layout = {}, ta.layout.bundle = function() {
		...
		return a.tick = function() {
			...
			if (g)
				for (Gu(t = ta.geom.quadtree(m), r, o), e = -1; ++e < b;)(a = m[e]).fixed || t.visit(n(a));
	...
	ta.layout.histogram = function() {
		...
		var t, e = ta.layout.hierarchy().sort(di),
			r = 0,
			u = [1, 1];
	...
	var kl = [2062260, 16744206, 2924588, 14034728, 9725885, 9197131, 14907330, 8355711, 12369186, 1556175].map(Mt),
		El = [2062260, 11454440, 16744206, 16759672, 2924588, 10018698, 14034728, 16750742, 9725885, 12955861, 9197131, 12885140, 14907330, 16234194, 8355711, 13092807, 12369186, 14408589, 1556175, 10410725].map(Mt),
		Al = [3750777, 5395619, 7040719, 10264286, 6519097, 9216594, 11915115, 13556636, 9202993, 12426809, 15186514, 15190932, 8666169, 11356490, 14049643, 15177372, 8077683, 10834324, 13528509, 14589654].map(Mt),
		Nl = [3244733, 7057110, 10406625, 13032431, 15095053, 16616764, 16625259, 16634018, 3253076, 7652470, 10607003, 13101504, 7695281, 10394312, 12369372, 14342891, 6513507, 9868950, 12434877, 14277081].map(Mt);
	ta.scale.quantile = function() {
		...
		function n() {
			...
			if ((p = Math.min(Math.abs(l - n) / 2, +u.apply(this, arguments))) > .001) {
				v = l > n ^ g ? 0 : 1;
				var T = null == S ? [_, w] : null == x ? [y, M] : Tr([y, M], [S, k], [x, b], [_, w]),
					R = y - T[0],
					D = M - T[1],
					P = x - T[0],
					U = b - T[1],
